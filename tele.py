...
inf = (y+'T'+a+'E'+b+'L'+y+'E'+m+'G'+c+'R'+r+'A'+y+'M'+'  '+y+'S'+a+'C'+b+'R'+y+'A'+m+'P'+c+'E'+r+'R'+y+'  '+'2'+y+'0'+a+'2'+b+'1'+y+'  '+m+'B'+c+'Y'+r+' '+y+'W'+m+'A'+c+'N'+r+'D'+y+'A')
el=0
def Sleep(timE):
    try:
        time.sleep(timE)
...
info()
def ospath():
    o=int(input(b+" Berapa akun telegram yang kamu punya ? : "))
    for po in range(o):
        ...
        with open('multi_log.txt', 'a') as f:
            f.write(api_id+'\n'+api_hash+'\n')
        client = TelegramClient("WandaHS{}".format(z), api_id, api_hash)
        client.start()
        Sleep(1)
        clear()
        info()
        client.disconnect()
if os.path.isfile('multi_log.txt'):
    xc=input(b+" mau melanjutkan session sebelumnya? "+a+" (y/n) ? ")
    if xc=='y':
        cy=input(" mau menambahkan akun? "+a+" (y/n) ? ")
        if cy=='y':
            ospath()
        else:
            pass
        ...
        if cv=='y':
            ...
            if con in ['', 'n']:
                ...
                Sleep(1)
                for d in range(v-1):
                    os.remove("WandaHS{}.session".format(d))
...
for s in range(v):
    ...
    Sleep(1)
    client = TelegramClient("WandaHS{}".format(x), api_id, api_hash)
    client.start()
    ...
    for chat in chats:
        try:
            ...
            continue
    print(b+' Pilih group untuk scrap member:')
    i=0
    for g in groups:
        print(m+str(i) +y+ ' - '+a + g.title)
        i+=1
    g_index = input(b+' pilih nomor (or press ENTER to skip): ')
    if g_index == '' :
        info()
        print(m+" Ok. skipping...")
        Sleep(1)
        ...
        Sleep(1)
    info()
    print(b+'Pilih group untuk menambahkan member:')
    i=0
    for group in groups:
        print(m+str(i) +y+ ' - ' +a+ group.title)
        i+=1
    g_index = input(b+' pilih nomor: ')
    if g_index=='':
